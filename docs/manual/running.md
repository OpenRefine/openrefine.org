...
# Any personal preferences with a JAVA_OPTIONS= line like for autosaving every 8 minutes
JAVA_OPTIONS=-Drefine.autosave=8
```
...
The project grid may display with both vertical and horizontal scrolling, depending on the number and width of columns, and the number of rows/records displayed. You can control the display of the project grid by using [Sort and View options](sortview).
